...
class Window:
    ...
    def is_shift_pressed():
        left, right = pr.is_key_down(pr.KEY_LEFT_SHIFT), pr.is_key_down(pr.KEY_RIGHT_SHIFT)
    ...
    @staticmethod
    def is_ctrl_pressed():
        left, right = pr.is_key_down(pr.KEY_LEFT_CONTROL), pr.is_key_down(pr.KEY_RIGHT_SHIFT)
        if left == True or right == True:
            return True
        else: return False
    ...
    def handle_escape_sequence(self, byte):
        ...
        # C1 Control Codes 
        if len(self.ESCBuffer) == 1 and 0x40 <= byteVal <= 0x5F: # byte > b"\x40" and byte < b"\x5f":
        ...
        if len(self.ESCBuffer) == 1 and (0x60 <= byteVal <= 0x7E or 0x20 <= byteVal <= 0x2F):
